...
const dataVersion = createHash("sha256").update(stableJson({ organizations, candidates, relationships, sources, observations, taxonomy })).digest("hex").slice(0, 16);
...
const bundle = { data_as_of:dataAsOf, data_version:dataVersion, schema_version:graph.schema_version, organizations, candidates, relationships, sources, observations, taxonomy };
await Promise.all([
  writeFile(path.join(root, "src", "data-version.js"), `// Generated by scripts/build-dataset.mjs; do not edit.\nexport const DATA_VERSION = "${dataVersion}";\n`),
